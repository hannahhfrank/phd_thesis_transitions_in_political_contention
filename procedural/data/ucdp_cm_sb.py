...
for country, group in ucdp_final[["gw_codes","best"]].groupby('gw_codes'):
    if group.iloc[0]['best'] > 0:
        ucdp_final.loc[group.index[0], 'onset'] = 1
    for i in range(1, len(group)):  
        if group.iloc[i]['best'] > 0 and group.iloc[i-1]['best'] == 0:
            ucdp_final.loc[group.index[i], 'onset'] = 1 
