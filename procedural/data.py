...
ucdp_sb_s = ucdp_sb[["year","dd","gw_codes","country","best","count","onset"]][~ucdp_sb['gw_codes'].isin(list(micro_states.values())+list(exclude.values())+list(exclude2.values()))]
ucdp_sb_s.columns=["year","dd","gw_codes","country","sb_fatalities","sb_event_counts","onset"]
...
gtd=pd.read_csv("data/data_out/gtd_cm_suicide_attacks.csv",index_col=0)
gtd_s=gtd.loc[gtd["year"]>=1989]
gtd_ss = gtd_s[["dd","gw_codes","n_suicide_attack","fatalities"]][~gtd_s['gw_codes'].isin(list(micro_states.values())+list(exclude.values())+list(exclude2.values()))]
gtd_ss.columns=["dd","gw_codes","terrorism_event_counts","terrorism_fatalities"]
...
df=pd.merge(left=df,right=gtd_ss,on=["dd","gw_codes"],how="left")
df=df.fillna(0)
...
# Final df
df_out=df[["year","dd","gw_codes","country","d_protest","d_riot","d_sb","d_osv","d_ns","d_terror","protest_event_counts","riot_event_counts","sb_fatalities","osv_fatalities","ns_fatalities","terrorism_fatalities","d_civil_conflict","d_civil_war","onset"]].copy()
print(df_out.isna().any())
df_out.to_csv("out/df_out_full_cm.csv") 
df_complete=df[["year","dd","gw_codes","country","d_protest","d_riot","d_sb","d_osv","d_ns","d_terror","protest_event_counts","riot_event_counts","sb_fatalities","osv_fatalities","ns_fatalities","terrorism_fatalities","d_civil_conflict","d_civil_war","onset"]].copy()
df_complete.to_csv("out/df_complete_cm.csv")
...
df_conf_hist=df_out[["year","dd","gw_codes","country","d_protest","d_riot","d_sb","d_osv","d_ns","d_terror","protest_event_counts","riot_event_counts","sb_fatalities","osv_fatalities","ns_fatalities","terrorism_fatalities"]].copy()
...
neighbors=pd.read_csv("data/data_out/cm_neighbors.csv",index_col=0)
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","protest_event_counts"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            ...
            if df_neighbors["protest_event_counts"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["protest_event_counts"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_proteset_event_counts_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","riot_event_counts"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["riot_event_counts"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["riot_event_counts"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_riot_event_counts_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","sb_fatalities"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["sb_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["sb_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_sb_fatalities_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","ns_fatalities"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["ns_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["ns_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_ns_fatalities_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","osv_fatalities"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["osv_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["osv_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_osv_fatalities_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","terrorism_fatalities"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")

df_neighbors["neighbors_terrorism"]=0
for i in range(len(df_neighbors)):
    print(i)
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        pass
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["terrorism_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["terrorism_fatalities"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
            df_neighbors.iloc[i, df_neighbors.columns.get_loc('neighbors_terrorism')] = counts

dichotomize(df_neighbors,"neighbors_terrorism","d_neighbors_terrorism_fatalities",0)
df_neighbors['d_neighbors_terror_fatalities_lag1'] = lag_groupped(df_neighbors,'country','d_neighbors_terrorism_fatalities',1)
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_terror_fatalities_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","d_civil_war"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["d_civil_war"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["d_civil_war"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_civil_war_lag1"]],on=["dd","gw_codes"],how="left")
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=df_out[["dd","country","gw_codes","d_civil_conflict"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
...
for i in range(len(df_neighbors)):
    ...
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["d_civil_conflict"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["d_civil_conflict"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
...
df_conf_hist=pd.merge(left=df_conf_hist,right=df_neighbors[["dd","gw_codes","d_neighbors_civil_conflict_lag1"]],on=["dd","gw_codes"],how="left")
...
# Load data 
demog=get_wb(list(range(1989, 2024, 1)),c_list,feat_dev,"wb_con_hist")
...
# Final df
df_conf_hist=df_conf_hist[["year","dd","gw_codes","country","d_protest_lag1","d_riot_lag1","d_riot_lag1","d_sb_lag1","d_osv_lag1","d_ns_lag1","d_terror_lag1","d_civil_war_lag1","d_civil_conflict_lag1","protest_event_counts_lag1","riot_event_counts_lag1","sb_fatalities_lag1","osv_fatalities_lag1","ns_fatalities_lag1","terrorism_fatalities_lag1","d_protest_zeros_decay","d_riot_zeros_decay","d_sb_zeros_decay","d_osv_zeros_decay","d_ns_zeros_decay","d_terror_zeros_decay",'d_civil_conflict_zeros_decay','d_civil_war_zeros_decay',"d_neighbors_proteset_event_counts_lag1","d_neighbors_riot_event_counts_lag1","d_neighbors_sb_fatalities_lag1","d_neighbors_osv_fatalities_lag1","d_neighbors_ns_fatalities_lag1","d_neighbors_terror_fatalities_lag1","d_neighbors_civil_war_lag1","d_neighbors_civil_conflict_lag1","regime_duration","pop_refugee",'pop_refugee_id']].copy()
print(df_conf_hist.isna().any())
print(df_conf_hist.min())
df_conf_hist.to_csv("out/df_conf_hist_full_cm.csv")  
df_complete=pd.read_csv("out/df_complete_cm.csv",index_col=0)
df_complete=pd.merge(left=df_complete,right=df_conf_hist[["dd","gw_codes","d_protest_lag1","d_riot_lag1","d_sb_lag1","d_osv_lag1","d_ns_lag1","d_terror_lag1","d_civil_war_lag1","d_civil_conflict_lag1","protest_event_counts_lag1","riot_event_counts_lag1","sb_fatalities_lag1","osv_fatalities_lag1","ns_fatalities_lag1","terrorism_fatalities_lag1","d_protest_zeros_decay","d_riot_zeros_decay","d_sb_zeros_decay","d_osv_zeros_decay","d_ns_zeros_decay","d_terror_zeros_decay",'d_civil_conflict_zeros_decay','d_civil_war_zeros_decay',"d_neighbors_proteset_event_counts_lag1","d_neighbors_riot_event_counts_lag1","d_neighbors_sb_fatalities_lag1","d_neighbors_osv_fatalities_lag1","d_neighbors_ns_fatalities_lag1","d_neighbors_terror_fatalities_lag1","d_neighbors_civil_war_lag1","d_neighbors_civil_conflict_lag1","regime_duration"]],on=["dd","gw_codes"],how="left")
df_complete.to_csv("out/df_complete_cm.csv")
...
# Load data 
demog=get_wb(list(range(1989, 2024, 1)),c_list,feat_dev,"wb_demog")
...
df_complete=pd.read_csv("out/df_complete_cm.csv",index_col=0)
df_complete=pd.merge(left=df_complete,right=df_demog[["dd","gw_codes",'pop','pop_density','pop_density_id','urb_share','rural_share','pop_male_share','pop_male_share_0_14','pop_male_share_15_19','pop_male_share_20_24','pop_male_share_25_29','pop_male_share_30_34','group_counts','group_counts_id','monopoly_share','monopoly_share_id','discriminated_share','discriminated_share_id','powerless_share','powerless_share_id','dominant_share','dominant_share_id','ethnic_frac','ethnic_frac_id','rel_frac','rel_frac_id','lang_frac','lang_frac_id','race_frac','race_frac_id']],on=["dd","gw_codes"],how="left")
df_complete.to_csv("out/df_complete_cm.csv")
...
# Load data 
economy = get_wb(list(range(1989, 2024, 1)),c_list,feat_dev,"wb_economy")
...
# Load data 
pol = get_wb(list(range(1989, 2024, 1)),c_list,feat_dev,"wb_pol")
...
base=df_out[["dd","gw_codes","country"]].copy()
base=pd.merge(left=base,right=reign[["dd",
                                    ...
                                    "election_recent", # Election for leadership taking place in that year
...
base_imp=imp_opti(base,"country",["tenure_months"],vars_add=["dem_duration","election_recent","lastelection"],max_iter=10,time="dd",last_train="2021-12")
...
base_imp_final=linear_imp_grouped(base,"country",["tenure_months"],time="dd",last_train="2021-12")
base_imp_final=simple_imp_grouped(base_imp_final,"country",["tenure_months"],time="dd",last_train="2021-12")
...
base_imp=imp_opti(base,"country",["dem_duration"],vars_add=["tenure_months","election_recent","lastelection"],max_iter=10,time="dd",last_train="2021-12")
...
base_imp_final=linear_imp_grouped(base,"country",["dem_duration"],time="dd",last_train="2021-12")
base_imp_final=simple_imp_grouped(base_imp_final,"country",["dem_duration"],time="dd",last_train="2021-12")
...
base_imp=imp_opti(base,"country",["election_recent"],vars_add=["tenure_months","dem_duration","lastelection"],max_iter=10,time="dd",last_train="2021-12")

# Calibrate 
base_imp_calib=calibrate_imp(base_imp, "country","election_recent")

### Simple ###

base_imp_final=linear_imp_grouped(base,"country",["election_recent"],time="dd",last_train="2021-12")
base_imp_final=simple_imp_grouped(base_imp_final,"country",["election_recent"],time="dd",last_train="2021-12")

# Merge
base_imp_final["election_recent"] = base_imp_final["election_recent"].fillna(base_imp_calib["election_recent"])
base_imp_final['election_recent_id'] = base_imp["missing_id"]
...
base_imp=imp_opti(base,"country",["lastelection"],vars_add=["tenure_months","dem_duration","election_recent"],max_iter=10,time="dd",last_train="2021-12")
...
base_imp_final=linear_imp_grouped(base,"country",["lastelection"],time="dd",last_train="2021-12")
base_imp_final=simple_imp_grouped(base_imp_final,"country",["lastelection"],time="dd",last_train="2021-12")
...
c_list=list(df_ccodes.iso_alpha3)
c_list = [char for char in c_list if char != "XYZ"]
geog = get_wb(list(range(1989, 2024, 1)),c_list,feat_dev,"wb_geog")
...
base_imp=imp_opti(base,"country",["AG.LND.FRST.ZS"],vars_add=["AG.LND.TOTL.K2","EN.GHG.CO2.MT.CE.AR5"],max_iter=10)
...
base_imp=imp_opti(base,"country",["EN.GHG.CO2.MT.CE.AR5"],vars_add=["AG.LND.TOTL.K2","AG.LND.FRST.ZS"],max_iter=10)
...
gw_codes=pd.read_csv("data/df_ccodes_gw.csv")
gw_codes_s=gw_codes.loc[gw_codes["end"]>=1989]
df_neighbors=pd.merge(left=ucdp_sb[["dd","country","gw_codes","best"]],right=neighbors[["gw_codes","dd","neighbors"]],on=["dd","gw_codes"],how="left")
df_neighbors["neighbors_fat"]=0
for i in range(len(df_neighbors)):
    if pd.isna(df_neighbors["neighbors"].iloc[i]): 
        pass
        ...
        for x in lst:
            c=int(gw_codes_s["gw_codes"].loc[gw_codes_s["country"]==x].iloc[0])
            if df_neighbors["best"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].empty==False:
                counts+=int(df_neighbors["best"].loc[(df_neighbors["dd"]==df_neighbors["dd"].iloc[i])&(df_neighbors["gw_codes"]==c)].iloc[0])
        if counts>0:
            df_neighbors.iloc[i, df_neighbors.columns.get_loc('neighbors_fat')] = counts

dichotomize(df_neighbors,"neighbors_fat","d_neighbors_con",0)
df_geog=pd.merge(left=df_geog,right=df_neighbors[["dd","gw_codes","d_neighbors_con"]],on=["dd","gw_codes"],how="left")
...
df_complete=pd.read_csv("out/df_complete_cm.csv",index_col=0)
df_complete=pd.merge(left=df_complete,right=df_geog[["dd",'gw_codes','land','temp','temp_id','forest','forest_id','co2','co2_id','percip','percip_id','waterstress','waterstress_id','agri_land','agri_land_id','arable_land','arable_land_id','rugged','soil','desert','tropical','cont_africa','cont_asia','d_neighbors_con','no_neigh','d_neighbors_non_dem','libdem_id_neigh']],on=["dd","gw_codes"],how="left")
df_complete.to_csv("out/df_complete_cm.csv")
