...
def multivariate_imp_bayes(df, country, vars_input, vars_add=None,max_iter=10,min_val=0):
    ...
    _=_.sort_values(by=["gw_codes","year"])
...
def multivariate_imp_neigh(df, country, vars_input, vars_add=None, max_iter=10,min_val=0):
...
def multivariate_imp_tree(df, country, vars_input,vars_add=None, max_iter=10,min_val=0):
...
def imp_opti(df, country, vars_input, vars_add=None, max_iter=10,min_val=0):
        
    _,emd_neigh=multivariate_imp_neigh(df,country,vars_input,vars_add,max_iter,min_val)
    _,emd_tree=multivariate_imp_tree(df,country,vars_input,vars_add,max_iter,min_val)
    _,emd_linear=multivariate_imp_bayes(df,country,vars_input,vars_add,max_iter,min_val)
...
def grouped_decay(df,group_var,var):
    result_df = df.groupby(group_var).apply(apply_decay,var=var)
...
def measure_time_since_last_civil_war(events):
    time_since_last_civil_war = np.zeros_like(events, dtype=int)  # Initialize array to store time since last civil war
    last_civil_war_index = None  # Initialize index of the last civil war occurrence
    
    for i, event_occurred in enumerate(events):
        if event_occurred:
            last_civil_war_index = i
        if last_civil_war_index is None:
            time_since_last_civil_war[i] = -1  # Use -1 as a sentinel value (or any other appropriate value)
        else:
            time_since_last_civil_war[i] = i - last_civil_war_index
    
    return time_since_last_civil_war
