...
names = pd.DataFrame(ucdp_ss.groupby(["year","country_id","country"])['side_b'].agg(lambda x: list(x.unique())))

### Merge fatality variables and reset index ----------------
best_high = pd.concat([best, names], axis=1)
best_high = pd.concat([best_high, high], axis=1)
...
fat = fat.reset_index()
fat["side_b"] = fat["side_b"].astype(str).str.replace(r"[\[\]']", "", regex=True)
fat["side_b"] = fat["side_b"].str.replace(r"\s*\(.*?\)", "", regex=True)  # Remove everything inside ()
    ...
    for x in range(0, len(years)):        
        ...
        if ((fat['year'] == years[x]) 
            & (fat['country_id'] == countries[i])).any() == False:
                s = {'year':fat['year'].loc[(fat["year"]==years[x])].iloc[0],'country':fat['country'].loc[(fat["country_id"]==countries[i])].iloc[0],'country_id':[countries[i]],'best':0,"side_b":"none",'high':0,'low':0,"count":0}
...
for i in range(0, len(add)):
        ...
        if (fat['gw_codes'] == add[i]).any() == False:
            for x in years:
                s = {'year':x,'country': df_ccodes[df_ccodes["gw_codes"]==add[i]]["country"].values[0],'gw_codes':add[i],'best':0,"side_b":"none",'high':0,'low':0,"count":0}
