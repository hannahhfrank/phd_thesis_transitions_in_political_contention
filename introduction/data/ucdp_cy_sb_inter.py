    ...
    for x in range(0, len(years)): 
        ...
        if ((fat['year'] == years[x]) 
            & (fat['country_id'] == countries[i])).any() == False:
                s = {'year':years[x],'country':fat['country'].loc[(fat["country_id"]==countries[i])].iloc[0],'country_id':[countries[i]],'best':0,'high':0,'low':0,"count":0}
...
ucdp_final.reset_index(drop=True,inplace=True)
ucdp_final.to_csv("data_out/ucdp_cy_sb_gov.csv",sep=',')
