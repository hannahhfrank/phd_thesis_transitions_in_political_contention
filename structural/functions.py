...
def multivariate_imp_bayes(df, country, vars_input, vars_add=None,max_iter=10,min_val=0,time="year",last_train=2013):
...
def multivariate_imp_neigh(df, country, vars_input, vars_add=None, max_iter=10,min_val=0,time="year",last_train=2013):
...
def multivariate_imp_tree(df, country, vars_input,vars_add=None, max_iter=10,min_val=0,time="year",last_train=2013):
...
def imp_opti(df, country, vars_input, vars_add=None, max_iter=10,min_val=0,time="year",last_train=2013):
...
def simple_imp_grouped(df, group, vars_input,time="year",last_train=2013):
...
def linear_imp_grouped(df, group, vars_input,time="year",last_train=2013):
...
def gen_model(y, x, target, inputs, name, names=None, model_fit=LinearRegression(), opti_grid=None, int_methods=False, inputs_plot=None, outcome="categorical",downsampling=False,time="year",last_train=2013):
    ...
    if downsampling==False: 
        ...
        for c in y.country.unique():
            ...
            # Train, test
            y_train = y_s[["country",time]+[target]].loc[y_s[time]<last_train]
            x_train = x_s[["country",time]+inputs].loc[x_s[time]<last_train]
            y_test = y_s[["country",time]+[target]].loc[(y_s[time]>=last_train)]
            x_test = x_s[["country",time]+inputs].loc[(x_s[time]>=last_train)]
            # Merge
    ...
    if opti_grid is not None:
        ...
        if downsampling==False:
            ...
            for c in y.country.unique():
                ...
                x_s = x.loc[x["country"] == c]
                y_train = y_s[["country",time]+[target]].loc[y_s[time]<last_train]
                x_train = x_s[["country",time]+inputs].loc[y_s[time]<last_train]
                train_y = pd.concat([train_y, y_train])
                train_x = pd.concat([train_x, x_train])
            train_y_d=train_y.drop(columns=["country",time])
            train_x_d=train_x.drop(columns=["country",time])
    ...
    if outcome == "continous":
        # TEST #
        t=out_df.loc[(out_df[time]>=2017)&(out_df[time]<=2023)]
        
        # MSE
        mse = mean_squared_error(t[target], t["preds"])
        # WMSE
        wmse = mean_squared_error(t[target], t["preds"], sample_weight=t[target])
        # EMD
        emd = earth_mover_distance(t[target], t["preds"])
        # Evals dictionary
        evals = {"mse": mse, "wmse": wmse, "emd": emd}
        
        # VAL #
        val=out_df.loc[(out_df[time]>=2013)&(out_df[time]<=2016)]
        ...
        # Evals dictionary
        evals_val = {"mse": mse, "wmse": wmse, "emd": emd}
        ...
        # AUPR
        precision, recall, _ = precision_recall_curve(t[target], t["preds_proba"])
    ...
    if int_methods == False:
        ...
        with open(f'/Users/hannahfrank/Dropbox/Apps/Overleaf/structural/out/struc_feat_shap_{name}.tex', 'w') as f:
            f.write(latex_table)
        with open(f'/Users/hannahfrank/Dropbox/Apps/Overleaf/PhD_dissertation/out/struc_feat_shap_{name}.tex', 'w') as f:
            f.write(latex_table)            
        with open(f'out/struc_feat_shap_{name}.tex', 'w') as f:
            f.write(latex_table)      
                
        # Plot, importances
        shap_importance = shap_importance.sort_values(by='Feature Shap Value', ascending=True)
        mask = shap_importance['Feature'].str.contains('nan imp')
        shap_importance = shap_importance[~mask]
        shap_importance = shap_importance[~mask].tail(7)
        fig,ax = plt.subplots(figsize=(5,len(shap_importance)/3))
        plt.barh(shap_importance['Feature'],shap_importance['Feature Shap Value'],color="black")
        ax.set_yticks(list(range(0,len(shap_importance))),list(shap_importance.Feature))
        rects = ax.patches
        ...
        plt.savefig(f"/Users/hannahfrank/Dropbox/Apps/Overleaf/structural/out/struc_feat_shap_{name}.png",dpi=300,bbox_inches='tight')
        plt.savefig(f"/Users/hannahfrank/Dropbox/Apps/Overleaf/PhD_dissertation/out/struc_feat_shap_{name}.png",dpi=300,bbox_inches='tight')
        plt.savefig(f"out/struc_feat_shap_{name}.png",dpi=300,bbox_inches='tight')
        plt.show()
        ...
        for count in range(0,len(inputs)):
            if len(train_x_d[inputs[count]].unique())==2:
                
                fig,ax = plt.subplots(figsize=(12, 8))
                x_vals=x_d.iloc[:, [count]].reset_index(drop=True)
                y_vals=pd.DataFrame(shap_values[:,:,1])[count].reset_index(drop=True)
                ...
                #x_vals=pd.DataFrame(x_d.iloc[:, [count]]).reset_index(drop=True)
                #y_vals=pd.DataFrame(shap_values[1])[count].reset_index(drop=True)
                #df_outliers=pd.concat([x_vals,y_vals],axis=1)
                #df_outliers.columns=["x","y"]
                #df_outliers_0=df_outliers.loc[df_outliers["x"]==0]
                #outlier_0_p = df_outliers_0.nlargest(3,["y"]).index.tolist()
                #outlier_0_n = df_outliers_0.nsmallest(3,["y"]).index.tolist()
                #df_outliers_1=df_outliers.loc[df_outliers["x"]==1]
                #outlier_1_p = df_outliers_1.nlargest(3,["y"]).index.tolist()
                #outlier_1_n = df_outliers_1.nsmallest(3,["y"]).index.tolist()          
                #outliers = list(set(outlier_0_p) | set(outlier_0_n) | set(outlier_1_p) | set(outlier_1_n))            
                # Add annotations 
                #for i in outliers:
                  #  plt.annotate(f'{x["country"].iloc[i]}--{x["year"].iloc[i]}', (x_vals.iloc[i], y_vals.iloc[i]), ha='center',)    
                
                plt.savefig(f"/Users/hannahfrank/Dropbox/Apps/Overleaf/structural/out/struc_shap_depend_{name}_{inputs[count]}.png",dpi=300,bbox_inches='tight')
                plt.savefig(f"/Users/hannahfrank/Dropbox/Apps/Overleaf/PhD_dissertation/out/struc_shap_depend_{name}_{inputs[count]}.png",dpi=300,bbox_inches='tight')
                plt.savefig(f"out/struc_shap_depend_{name}_{inputs[count]}.png",dpi=300,bbox_inches='tight')           
                plt.show()   
        
            else:
                #model = SVR(kernel='rbf')
                #model.fit(x_d.iloc[:, [count]],pd.DataFrame(shap_values[1])[count])
                #x_pred = np.linspace(x_d.iloc[:, [count]].min(), x_d.iloc[:, [count]].max(), 100).reshape(-1, 1)
                #y_pred = model.predict(x_pred)
                
                fig,ax = plt.subplots(figsize=(12, 8))
                plt.scatter(x_d.iloc[:, [count]],pd.DataFrame(shap_values[:,:,1])[count],color="black",s=60)
                plt.xlabel(names[inputs[count]],size=20)
                plt.ylabel("SHAP value",size=20)
                plt.yticks(size=20)
                plt.xticks(size=20)
                sns.rugplot(x_d[inputs[count]], height=0.05, color='black')
                #plt.plot(x_pred, y_pred, color='gray',linewidth=4)

                #x_vals=pd.DataFrame(x_d.iloc[:, [count]]).reset_index(drop=True)
                #y_vals=pd.DataFrame(shap_values[1])[count].reset_index(drop=True)
                #outlier_x_p = x_vals.nlargest(3,[inputs[count]]).index.tolist()
                #outlier_x_n = x_vals.nsmallest(3,[inputs[count]]).index.tolist()
                #outlier_y_p = y_vals.nlargest(3).index.tolist()
                #outlier_y_n = y_vals.nsmallest(3).index.tolist()
                #df_outliers=pd.concat([x_vals,y_vals],axis=1)
                #df_outliers.columns=["x","y"]
                #outliers_x_y_p=df_outliers.nlargest(3,["x","y"]).index.tolist()
                #outliers_x_y_n=df_outliers.nsmallest(3,["x","y"]).index.tolist()
                #outliers = list(set(outlier_x_p) | set(outlier_x_n) | set(outlier_y_p) | set(outlier_y_n) | set(outliers_x_y_p) | set(outliers_x_y_n))
                # Add annotations 
                #for i in outliers:
                  #  plt.annotate(f'{x["country"].iloc[i]}--{x["year"].iloc[i]}', (x_vals.iloc[i], y_vals.iloc[i]), ha='center',)    
